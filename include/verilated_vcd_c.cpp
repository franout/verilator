...
constexpr unsigned VL_TRACE_MAX_VCD_CODE_SIZE = 32;  // Maximum length of a VCD string code
...
// cache-lines.
constexpr unsigned VL_TRACE_SUFFIX_ENTRY_SIZE = 16;  // Size of a suffix entry
...
void VerilatedVcd::open(const char* filename) VL_MT_SAFE_EXCLUDES(m_mutex) {
    ...
    // hardcoded string for date
    printStr("$date\n    Feb 3, 2024  23:34:11\n$end\n");
    printStr("$version\n    Generated by VerilatedVcdC customized\n$end\n");
    printStr("$timescale\n    ");
...
void VerilatedVcd::declare(uint32_t code, const char* name, const char* wirep, bool array,
    ...
    char vcdCode[VL_TRACE_SUFFIX_ENTRY_SIZE];
    {
        ...
        do {
            *vcdCodeWritep++ = static_cast<char>('0' + codeEnc % 94);
            codeEnc /= 94;
        } while (codeEnc--);
        *vcdCodeWritep = '\0';
        ...
        // this format never has a separator ' ' between scalar or bus
        //char* entryWritep = bits == 1 ? entryBeginp : entryBeginp + 1;
        char* entryWritep = entryBeginp + 1;
    ...
    decl += " $end\n";
    //printIndent(0); no need, it the vcd parser does not like them
    printStr(decl.c_str());
}
...
void VerilatedVcd::declBit(uint32_t code, uint32_t fidx, const char* name, int dtypenum,
                           VerilatedTraceSigDirection, VerilatedTraceSigKind,
                           VerilatedTraceSigType, bool array, int arraynum) {
    declare(code, name, "port", array, arraynum, false, 0, 0);
}
void VerilatedVcd::declBus(uint32_t code, uint32_t fidx, const char* name, int dtypenum,
    ...
    declare(code, name, "port", array, arraynum, true, msb, lsb);
}
